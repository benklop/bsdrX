...
"<option value=1>GPU — NVENC / VAAPI (offload / high bitrate)</option></select>"
"<span class=hint style=margin-left:6px>x264 keeps low-bitrate text crisp; GPU encode uses NVENC when "
"NVIDIA is present, otherwise VAAPI (Intel Arc / iGPU / AMD). Saved across restarts.</span></div>"
"<div class=row><label style=width:auto;color:var(--muted)>Intel/AMD GPU (Linux)</label>"
...
"<span class=hint style=margin-left:6px>Linux: <b>VAAPI</b> encodes on Intel Arc / iGPU or AMD (also "
"the automatic GPU path when no NVIDIA card is present). <b>kmsgrab</b> grabs the display GPU via "
"DRM/KMS — zero-copy with VAAPI, needs <code>CAP_SYS_ADMIN</code> "
"(<code>setcap cap_sys_admin+ep</code>). Applied live (fresh keyframe); falls back if unavailable.</span></div>"
...
"<div class=row><input class=grow readonly value='bc1q3zlkpu95amtcltsk85y0eacyzzk29v68tgc5hx' onclick=this.select()>"
"<button style=width:auto onclick=\"dnCopy(this,'bc1q3zlkpu95amtcltsk85y0eacyzzk29v68tgc5hx')\">Copy</button></div></div>"
"<div class=sub2><div class=t>&#926; Ethereum (ETH)</div>"
"<div class=row><input class=grow readonly value='0x76C00887C250c08f4E063638e0aF2ac745Fcc0D1' onclick=this.select()>"
"<button style=width:auto onclick=\"dnCopy(this,'0x76C00887C250c08f4E063638e0aF2ac745Fcc0D1')\">Copy</button></div></div>"
"<div class=sub2><div class=t>&#9678; Solana (SOL)</div>"
"<div class=row><input class=grow readonly value='2o6bKUHyMuHqUoR3RwaMrQW6ZaDy9CwUKkejFSMKeFv2' onclick=this.select()>"
"<button style=width:auto onclick=\"dnCopy(this,'2o6bKUHyMuHqUoR3RwaMrQW6ZaDy9CwUKkejFSMKeFv2')\">Copy</button></div></div>"
...
"var _ch=dot(s.cloud.loggedIn)+'<span>'+(s.cloud.loggedIn?('Connected as '+s.cloud.name):('Not connected — '+s.cloud.msg))+'</span>'+(s.cloud.loggedIn?'<button onclick=logout() style=margin-left:auto>Log out</button>':'');if(cloud.innerHTML!==_ch)cloud.innerHTML=_ch;"
