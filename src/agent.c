...
"  --gpu                    GPU encode: CUDA/NVENC when NVIDIA is present, otherwise\n"
"                           VAAPI (Intel Arc / iGPU / AMD). Default on Windows/macOS.\n"
"                           Saved settings and this flag persist across restarts;\n"
"                           an explicit --cpu/--gpu on the command line wins.\n"
"  --lan-1x                 Send LAN video once instead of 2x — halves the uplink\n"
"                           on a weak WiFi link (default: 2x, like the official host).\n"
"  --vaapi                  Encode via VAAPI (Intel Arc/iGPU iHD, AMD radeonsi) instead\n"
"                           of NVENC. On a box with no NVIDIA GPU this is already the\n"
"                           GPU-encode path. Falls back to CPU on failure.\n"
"  --kmsgrab                Capture via DRM/KMS instead of x11grab (zero-copy with\n"
"                           VAAPI). Picks the display GPU, not always card0. Needs\n"
"                           CAP_SYS_ADMIN: setcap cap_sys_admin+ep build/bsdr_agent.\n"
